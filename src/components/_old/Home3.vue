<template>
  <div class="home-container">
    ...
    <div v-if="isLoading && numberOfFetchedUsers === 0">
      <MockFeed :count=10 />
    </div>
    ...
    <div v-if="users && users.length > 0" class="user-grid">
      ...
      <!-- <div v-if="isLoading && numberOfFetchedUsers > 0"> -->
        <MockFeed :count="numberOfFetchedUsers" />
      <!-- </div> -->

    </div>

    <!-- Show mock cards while more data is loading -->
    <!-- <div>{{ numberOfFetchedUsers }}</div>
     <div>{{ isLoading }}</div> -->
...
import MockFeed from './MockFeed.vue';
...
export default {
  ...
  data() {
    return {
      ...
      showSnapIndicator: false, // Controls visibility of the snap-up indicator
      snapTriggered: false, // Indicates whether the snap-up effect is active
      isLoading: true,
      fetchedUsers: 0,
      users: [],
      userLocation: {
  ...
  mounted() {
    ...
    // this.debouncedCheckScroll = this.debounce(this.checkScroll, 100);
  ...
  methods: {
    ...
    async fetchNearbyUsers() {
      ...
      try {
        ...
        console.log('Number of fetched users: ', response.data?.length);
        ...
        // Ensure there are no duplicate users in the list
    ...
    // },
    checkScroll() {
      const offset = 20; // The point at which you want to trigger the load
      const scrollPosition = window.innerHeight + window.scrollY;
      const documentHeight = document.documentElement.offsetHeight;
      console.log('Scroll position = ' + scrollPosition)

      // Trigger loading when the scroll is within `offset` pixels from the bottom
      if (scrollPosition >= documentHeight - offset && !this.snapTriggered) {
        console.log('Trigger loading and snap-up indicator');
